@extends('layouts.app')
@section('title', 'Edit Contingent')
@php $header = 'Edit Contingent'; @endphp
    ...
    <form action="{{ route('contingents.update', $contingent) }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <select name="event_id" id="event_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('event_id') border-red-500 @enderror" required>
                    @foreach($events as $event)
                        <option value="{{ $event->id }}" {{ old('event_id', $contingent->event_id) == $event->id ? 'selected' : '' }}>Event #{{ $event->id }} ({{ $event->start_date->format('M d, Y') }})</option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center justify-end space-x-3 pt-4 border-t">
                ...
                <button type="submit" class="inline-flex items-center rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500">Update Contingent</button>
            </div>
        </div>
    </form>
</div>
