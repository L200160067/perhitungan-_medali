...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            <a href="{{ route('dojangs.index') }}" class="text-blue-600 hover:text-blue-800 text-sm">← Kembali ke Dojang</a>
        </div>
        <div class="space-x-2">
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">Informasi Dojang</h2>
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <dd class="mt-1 text-sm text-gray-900">{{ $dojang->created_at->format('d M Y H:i') }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Diperbarui Pada</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $dojang->updated_at->format('d M Y H:i') }}</dd>
            </div>
        </dl>
    </div>
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-lg font-semibold text-gray-900">Peserta ({{ $dojang->participants->count() }})</h2>
            <a href="{{ route('participants.create', ['dojang_id' => $dojang->id]) }}" class="text-sm text-blue-600 hover:text-blue-800">+ Tambah Peserta</a>
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tanggal Lahir</th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $participant->gender->label() }}</td>
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Kontingen ({{ $dojang->contingents->count() }})</h2>
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nama</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Pertandingan</th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <a href="{{ route('events.show', $contingent->event) }}" class="text-blue-600 hover:text-blue-800">{{ $contingent->event->name ?? 'Pertandingan #' . $contingent->event_id }}</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
