<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="space-y-6">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <p class="text-gray-600 dark:text-gray-400">Pilih event untuk memperbarui perolehan medali atlet.</p>
                    </div>
                    ...
                    <a href="{{ route('registrations.index') }}" 
                       class="relative flex flex-col p-4 rounded-xl border-2 transition-all duration-200 group {{ !$eventId ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 ring-1 ring-blue-500' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-blue-300 dark:hover:border-blue-500 hover:shadow-md' }}">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-bold uppercase tracking-wider {{ !$eventId ? 'text-blue-700 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-400' }}">Semua Event</span>
                            @if(!$eventId)
                                <svg class="h-5 w-5 text-blue-600 dark:text-blue-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>
                            @endif
                        </div>
                        <p class="text-xs {{ !$eventId ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500 group-hover:text-gray-500 dark:group-hover:text-gray-400' }}">Tampilkan semua data pendaftaran</p>
                    </a>
                    ...
                    @foreach($events as $event)
                        <a href="{{ route('registrations.index', ['event_id' => $event->id]) }}" 
                           class="relative flex flex-col p-4 rounded-xl border-2 transition-all duration-200 group {{ $eventId == $event->id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 ring-1 ring-blue-500' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-blue-300 dark:hover:border-blue-500 hover:shadow-md' }}">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-bold uppercase tracking-wider {{ $eventId == $event->id ? 'text-blue-700 dark:text-blue-300' : 'text-gray-600 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-400' }} truncate">{{ $event->name }}</span>
                                @if($eventId == $event->id)
                                    <svg class="h-5 w-5 text-blue-600 dark:text-blue-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>
                                @endif
                            </div>
                            <div class="flex items-center gap-2 text-xs {{ $eventId == $event->id ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500 group-hover:text-gray-500 dark:group-hover:text-gray-400' }}">
    ...
    @if($eventId && ($activeEvent = $events->find($eventId)))
        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-6 flex flex-col lg:flex-row justify-between items-center gap-6 shadow-sm">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    <td class="px-4 py-4 whitespace-nowrap text-xs text-gray-500 dark:text-gray-400 font-medium">{{ $loop->iteration }}</td>
                    <td class="px-4 py-4 text-sm text-gray-900 dark:text-white min-w-[200px]">
                        <a href="{{ route('tournament-categories.show', $registration->category) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-semibold leading-tight block transition">
                    ...
                    <td class="px-4 py-4 text-sm text-gray-900 dark:text-white font-medium min-w-[150px]">{{ $registration->participant->name }}</td>
                    <td class="px-4 py-4 text-sm text-gray-900 dark:text-white min-w-[180px]">
                        <a href="{{ route('contingents.show', $registration->contingent) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 leading-tight block transition">
                            {{ $registration->contingent->name }}
                        </a>
                    </td>
                    <td class="px-4 py-4 whitespace-nowrap text-center text-sm">
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $registration->status->color() }}">
                    ...
                    <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                        ...
                        <a href="{{ route('registrations.edit', $registration) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 transition">Edit</a>
                ...
                @empty
                <tr>
                    <td colspan="8" class="px-4 py-12 text-center text-gray-500 dark:text-gray-400">Pendaftaran tidak ditemukan</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="bg-white dark:bg-gray-800 px-4 py-3 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 sm:px-6 rounded-b-lg shadow-sm">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div class="flex items-center gap-4">
                <p class="text-sm text-gray-700 dark:text-gray-400">
                    Menampilkan
                    <span class="font-medium dark:text-gray-200">{{ $registrations->firstItem() ?? 0 }}</span>
                    sampai
                    <span class="font-medium dark:text-gray-200">{{ $registrations->lastItem() ?? 0 }}</span>
                    dari
