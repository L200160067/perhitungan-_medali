...
<div class="max-w-2xl">
    <form action="{{ route('registrations.store') }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        ...
        <div class="space-y-6">
            ...
            <div><label for="participant_id" class="block text-sm font-medium text-gray-700">Participant <span class="text-red-500">*</span></label><select name="participant_id" id="participant_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('participant_id') border-red-500 @enderror" required><option value="">Select a participant</option>@foreach($participants as $participant)<option value="{{ $participant->id }}" {{ old('participant_id') == $participant->id ? 'selected' : '' }}>{{ $participant->name }}</option>@endforeach</select>@error('participant_id')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
