        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="space-y-6">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <p class="text-gray-600 dark:text-gray-400">Kelola semua peserta</p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                        ...
                        @role('admin')
                        <a href="{{ route('participants.import') }}" class="inline-flex items-center justify-center rounded-md bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-500 transition">
                            Import Excel
                        </a>
                        <a href="{{ route('participants.create', request()->query()) }}" class="inline-flex items-center justify-center rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 transition">
                            + Baru
                        </a>
                        @endrole
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden" x-data="{ 
                    ...
                    <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                        <form id="bulk-delete-form" action="{{ route('participants.bulkDestroy') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data yang dipilih?')">
                            @csrf
                            @method('DELETE')
                                ...
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left sticky top-0 z-10 bg-gray-50 dark:bg-gray-700">
                                        <input type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" 
                                            @change="
                                                $el.checked ? 
                                                selected = [{{ $participants->pluck('id')->implode(',') }}] : 
                                                selected = [];
                                                updateCount();
                                            ">
                                    </th>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400 min-w-[200px]">
                                        <a href="{{ route('dojangs.show', $participant->dojang) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition leading-tight block">
                                            {{ $participant->dojang->name }}
                                        </a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $participant->gender->value === 'M' ? 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200' : 'bg-pink-100 dark:bg-pink-900 text-pink-800 dark:text-pink-200' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                        <a href="{{ route('participants.show', $participant) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 transition">Lihat</a>
                                        <a href="{{ route('participants.edit', ['participant' => $participant] + request()->query()) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 transition">Edit</a>
                                        @role('admin')
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div class="flex items-center gap-4">
                                <p class="text-sm text-gray-700 dark:text-gray-400">
                                    Menampilkan
                                    <span class="font-medium dark:text-gray-200">{{ $participants->firstItem() ?? 0 }}</span>
                                    sampai
                                    <span class="font-medium dark:text-gray-200">{{ $participants->lastItem() ?? 0 }}</span>
                                    dari
                                    <span class="font-medium dark:text-gray-200">{{ $participants->total() }}</span>
                                    data
                                </p>
