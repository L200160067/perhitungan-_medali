...
@section('content')
<div class="max-w-2xl">
    <form action="{{ route('participants.update', $participant) }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                >
                    @foreach($dojangs as $dojang)
                        <option value="{{ $dojang->id }}" {{ old('dojang_id', $participant->dojang_id) == $dojang->id ? 'selected' : '' }}>
                            {{ $dojang->name }}
                        </option>
                    @endforeach
                </select>
